...
class Account:
    ...
    def get_account_by_id(cls, data):
        ...
        account = connectToMySQL('private_wall').query_db(query, data)
        return account
    
    @classmethod
    def get_account_exclude_id(cls, data):
        query = 'SELECT * FROM account WHERE id <> %(id)s ORDER BY first_name;'
        account_list = connectToMySQL('private_wall').query_db(query, data)
        return account_list
    ...
    def insert_one(cls, data):
        ...
        new_account_id = connectToMySQL('private_wall').query_db(query, data)
